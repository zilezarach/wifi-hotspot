...
class RB951HotspotManager {
  ...
  async grantAccessByIP(ip: string, duration: string): Promise<{ success: boolean; message: string }> {
    try {
      const client = await this.connect();
  ...
  async grantAccessByMAC(mac: string, duration: string): Promise<{ success: boolean; message: string }> {
    try {
      const client = await this.connect();
  ...
  async disconnectUser(ip: string): Promise<{ success: boolean; message: string }> {
    try {
      const client = await this.connect();
